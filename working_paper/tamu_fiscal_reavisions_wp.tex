...
To further explore whether governments are more likely to stretch the rules closer to elections and whether this effect interacts with financial market stress, we use Gandrud's \citeyearpar{gandrudYrcurnt} \emph{years to election} variable in the following regressions. The variable counts down from the year that is the furthest away based on national election requirements. Election years are recorded as zero. To make the following marginal effect estimates more intuitive--i.e. so that a positive coefficient would indicate that being closer to an election was associated with an increase in revisions, we reversed the scale.
